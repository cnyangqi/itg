...
public class CartAction extends DispatchAction {
	...
	/*
	 * 显示购物车
	 * 当session有order的时候去订单明细表里查(如果session中有order的时候说明用户一定是登陆的，因为用户在确认地址时会生成订单
	 * ，并把相应订单号存在session中) 当用户登陆的时候去数据库查找 当用户没有登陆的时候在cookie中查找
	...
	public ActionForward view(	ActionMapping mapping,
		...
		try {
			...
			if (user != null) {
				String session_order = JUtil.convertNull((String) request.getSession()
																			.getAttribute("order"));
	...
	/*
	 * 添加产品到购物车当用户登陆的时候存到数据库没有登陆的时候添加到cooke中
	 */
	...
	/*
	 * 删除购物车的产品 当用户登陆的时候从数据库中删除 当用户没有登陆的时候添加数据库cooke中
	 */
			...
			if (TokenManager.isTokenValid(request, true)) {
				...
				if (user != null) {
					...
					if (cart_ids != null && cart_ids.length > 0) {
						...
						if (_ls != null)
							request.setAttribute("beans", _ls);
					} else {
						request.setAttribute("beans", ls);
					}
				}
