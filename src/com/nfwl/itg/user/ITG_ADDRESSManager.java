...
public class ITG_ADDRESSManager {
	...
	public ITG_ADDRESS insert(Connection con,ITG_ADDRESS bean) throws JadoException{
		try{
			con.setAutoCommit(false);
			this.bean=bean;
			this.initBeanInfo();
			if(this.getBean().getAdr_id()==null||this.getBean().getAdr_id().equals("")) this.getBean().setAdr_id(JUtil.createUNID());
			od.insert(con);
			//更新默认地址
			UserManager um = new UserManager();
			um.updateAddress(con, this.bean.getAdr_userid(), bean.getAdr_id());
			con.commit();
	...
	public List<Bean> getByUser(Connection con,String user_id,int start,int size)throws JadoException{
		this.bean=null;
		this.initBeanInfo();
			...
			if(ids!=null){
				...
				if(user!=null&&user.getId().equals(ids.getAdr_userid())){
					if(user.getAdrid()!=null&&user.getAdrid().equals(ids.getAdr_id())){
						um.updateAddress(con, ids.getAdr_userid(), "");
					}
					this.bean=null;
					this.initBeanInfo();
					this.bean.setAdr_id(id);
	...
	public boolean setDefault(Connection con,String adr_id,String user_id)throws JadoException{
		UserManager um = new UserManager();
		return um.updateAddress(con, user_id, adr_id);
	}
	private void initBeanInfo() throws JadoException{
		if(this.bean==null) bean = new ITG_ADDRESS();
