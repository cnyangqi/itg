...
public class LoadAddressAction extends nfwlAction {
	...
	public ActionForward orderAddress(	ActionMapping mapping,
										ActionForm form,
	...
	public ActionForward view(	ActionMapping mapping,
		...
		try {
			...
			// nps.core.User user = (nps.core.User)
	...
	public ActionForward delete(ActionMapping mapping,
		...
		try {
			...
			if (TokenManager.isTokenValid(request, true)) {
				...
				String id = JUtil.convertNull(request.getParameter("id"));
				if (!id.equals(""))
					iam.delete(con, id, user.getId(), user.getType());
	...
	/** 设置用户默认收货地址 */
	public ActionForward setdefault(ActionMapping mapping,
									ActionForm form,
									HttpServletRequest request,
									HttpServletResponse response) throws Exception {
		Connection con = null;
		...
		try {
			if (TokenManager.isTokenValid(request, true)) {
				...
				String id = JUtil.convertNull(request.getParameter("id"));
				if (!id.equals("")) {
					boolean flag = iam.setDefault(con, id, user.getId(), user.getType());
					if (flag) {
						Message ms = new Message();
						ms.setFlag(true);
						ms.setMsg("设置默认地址成功!");
