...
public class OrderController extends ApplicationController {
	 ...
	 * @throws NpsException
	 */
	public static void carOut(Connection con, String fpids, int fromstatus, int tostatus)
			throws NpsException {
	 ...
	 * @throws NpsException
	 */
	public static void carOutOrder(Connection con, String caroutid, String fpid)
			throws NpsException {
	...
	public View changeOrder(String ids, String todo) throws NpsException {
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = "update  itg_orderrec set or_status = "
						+ todo
						+ " where or_id in ("
						+ ids
						+ ")";
	 ...
	 * @throws NpsException
	 */
	private ArrayList<OrderModels> getOrders(Connection con, Integer status) throws JError,
			NpsException {
		...
		try {
			...
			while (rs.next()) {
				if (alods == null)
					alods = new ArrayList<OrderModels>();
				order = new OrderModels();
				order.setOr_id(rs.getString("or_id"));
				order.setOr_userid(rs.getString("or_userid"));
				order.setOr_money(rs.getDouble("or_money"));
