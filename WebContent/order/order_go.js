...
function chk_adr_click(obj, detail, postcode, name, mobile, email, areacode,
		telephone, subnum, zone) {
...
function numChange(obj, point, price) {
	var num = obj.value;
	if (!isNumeric(num)) {
		num = 1;
	}
	...
	$("#priceSpan").html(num * price);
	$("#pointSpan").html(num * point);
}
...
function callback() {
	$("#ul_submit")
			.html(
					'<a href="javascript:ordergofrmSubmit()"><img src="/images/submit_06.png" /></a>');
	ordergofrmSubmit();
}
