...
<%
    ...
    try
    {
        conn = Database.GetDatabase("nfwl").GetConnection();
        ...
        sql = "select count(*) from itg_productimage where 1=1 ";
        ...
        if (totalrows > 0)
        {
            ...
            sql = "select  pi_filename caption,pi_ext,'' rooturl,'/'||pi_filepath||pi_id||pi_ext img_rooturl ,'' name,  pi_time createdate,''  creator_cn from  itg_productimage";
            pstmt = conn.prepareStatement(sql);
  ...
  <table width = "100% " border = "0" cellpadding = "0" cellspacing = "1">
   ...
   %>
                   <td width="<%=100/IMAGES_PER_ROW%>%" align=center>
                       <%
                           String img_url = rs.getString("img_rooturl");
                           ...
                           if(img_url==null)
                           {
                               ...
                               img_url = site_image_rooturl
                                          ...
                                          + rs.getString("img_rooturl") + Utils.Null2Empty(rs.getString("pi_ext"));

                               img_preview_url = "/userdir/preview/"
                                          + Utils.FormateDate(rs.getDate("createdate"),"yyyy/MM/dd/")
                                          + rs.getString("img_rooturl") + ".jpg";
                           }
                           else
                           {
                       ...
                       <table border="0" width="100%" cellpadding="0" cellspacing="0">
                           <input type="hidden" name="rsid_<%=rs.getRow()%>" value="<%=rs.getString("img_rooturl")%>">
                           <input type="hidden" name="url_<%=rs.getRow()%>" value="<%=img_url%>">
                           <input type="hidden" name="name_<%=rs.getRow()%>" value="<%=rs.getString("caption")%>">
                           <input type="hidden" name="s_<%=rs.getRow()%>" value="<%=rs.getString("pi_ext")%>">
   ...
   %>
          ...
          %>
                  <tr height=25 class="DetailBar">
                    ...
                    <td>
                       <%=Utils.Null2Empty(rs.getString("pi_ext"))%>
                    </td>
                    <td>
                       <%=rs.getString("siteid")%>
                    </td>
