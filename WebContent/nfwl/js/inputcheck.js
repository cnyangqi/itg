...
function CheckIsNull(Inform, Inputname) {
	...
	if (Temp == "") {
		...
		eval(Form + Inputname + ".className=" + Form + Inputname
				+ ".className.replace('RedInput','');");
		return true;
	}
}
// 验证是否为数字
...
function CheckIsHttp(Inform, Inputname) {
	if (!CheckIsNull(Inform, Inputname))
		...
		var Form = Inform + ".";
		eval("Temp=" + Form + Inputname + ".value;");
		if (Temp.search(/^http:\/\/([\w-]+\.)+[\w-]+(\/[\w- .\/?%&=]*)?/) == -1) { // alert("提醒您:不为HTTP");
			eval(Form + Inputname + ".className='RedInput';");
			eval(Form + Inputname + ".focus();");
			return false;
		} else {
...
function CheckIsMobile(Inform, Inputname) {
	if (!CheckIsNull(Inform, Inputname))
		...
		var Form = Inform + ".";
		eval("Temp=" + Form + Inputname + ".value;");
		if (Temp.search(/^1[3|5]\d{9}$/) == -1) { // alert("提醒您:不为手机号码");
			eval(Form + Inputname + ".className='RedInput';");
			eval(Form + Inputname + ".focus();");
			return false;
		} else {
