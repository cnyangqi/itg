...
<%
    ...
    String scrollstr = "", nextpage = "selectproduct.jsp";
    ...
    if(site_id==null || site_id.length()==0) site_id = "itg";//throw new NpsException(ErrorHelper.INPUT_NOTENOUGH);
    scrollstr = "site_id="+site_id;

    String top_id = request.getParameter("top_id");
    if(top_id==null || top_id.length()==0) top_id = "101";//throw new NpsException(ErrorHelper.INPUT_NOTENOUGH);
	...
	<HEAD>
        ...
        <script langauge = "javascript">
            ...
            function doChoose(){
              ...
              var index = 0;
              var prdidvalue = '',prdnamevalue = '',prdunitvalue = '';
              for (var i = 0; i < rownos.length; i++)
              {
                if(rownos[i].checked){
                  index++;
                  prdidvalue = rownos[i].prdid;
                  prdnamevalue = rownos[i].prdname;
                  prdunitvalue = rownos[i].prdunit;
                }
              }
              ...
              opener.setSubProduct('<%=prdid%>',prdidvalue,'<%=prdname%>',prdnamevalue,'<%=prdunit%>',prdunitvalue);
              window.close();
          }
...
%>
<form name="frm" method="post" action="selectproduct.jsp">

    <input type="hidden" name="prdid" value="<%=prdid%>">
    <input type="hidden" name="prdname" value="<%=prdname%>">
    <input type="hidden" name="prdunit" value="<%=prdunit%>">
  ...
  <table width = "100% " border = "0" cellpadding = "0" cellspacing = "0" class="PositionBar">
    <tr>
      <td valign="middle">&nbsp;
        ...
        <input type="button" class="button" name="chooseBtn" value="<%=bundle.getString("SELECTART_CHOOSEBUTTON")%>" onclick="doChoose()">
      </td>
      <td align="right">
...
<%
        if (totalrows > 0)
        {
            ...
            if(table_name.equals("article"))
                sql = "select a.Id,a.prd_name, a.prd_unitid,a.prd_unitname,a.title,a.creator_name creator,a.createdate,a.publishdate,b.id top_id,b.Name top_name,c.id site_id,c.name site_name from article a,topic b,site c Where a.topic=b.Id And a.siteid=c.Id ";
            else
                sql = "select a.Id,a.prd_name, a.prd_unitid,a.prd_unitname,a.title,a.createdate,a.publishdate,b.id top_id,b.Name top_name,c.id site_id,c.name site_name from " + table_name + " a,topic b,site c Where a.topic=b.Id And a.siteid=c.Id ";
            ...
            String articleId = "",vprdid = "",vprdname = "",vprdunit = "", siteId = null, topId = null;
            rownum = 0;
            while (rs.next() && (rs.getRow() <= endnum))
            {
                ...
                vprdid = rs.getString("id");
                vprdname = rs.getString("prd_name");
                vprdunit = rs.getString("prd_unitname");
%>
              <tr class="DetailBar">
                <td>
                  <input type = "checkBox" id="rowno" name="rowno"   prdid="<%= articleId %>" prdname="<%= vprdname %>" prdunit="<%= vprdunit %>"   value = "<%= rs.getRow() %>">
