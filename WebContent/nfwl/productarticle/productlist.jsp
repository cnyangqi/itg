...
<HTML>
	<HEAD>
        ...
        <script langauge = "javascript">
            ...
            {
                document.listFrm.action = "productlist.jsp?job=copy&copy_site="+siteid+"&copy_topic="+topid;
           ...
           {
               document.listFrm.action = "productlist.jsp?job=addtopic&slave_site="+siteid+"&slave_topic="+topid;
               document.listFrm.target = "_self";
               document.listFrm.submit();
           }
            ...
            document.listFrm.action = "productlist.jsp?job=cancel";
                ...
                if( actiontype == 'down')
                {
            document.listFrm.action = "productlist.jsp?job=down";
                    document.listFrm.target="_self";
                }
                if( actiontype == 'up')
                {
            document.listFrm.action = "productlist.jsp?job=up";
                    document.listFrm.target="_self";
                }
        ...
        switch(status)
        {
            ...
            case 9: //已手动下架
                ...
                if(top_id!=null && top_id.length()>0)
                {
                    ...
                    {
                        sql += " and a.id in (" +
                            "    select w.id from article w,topic y where  y.is_business = 1 and  w.topic=y.id and (y.id=? or y.code like ?) " +
                            "union " +
                            "    select z.artid from topic y,article_topics z where y.id=z.topid and y.siteid=z.siteid and  y.is_business = 1 " +
                            "    and (y.id=? or y.code like ?)" +
                            ")";
                    }
                    else
                    {
                        sql += " and a.id in (select w.id from "+ table_name + " w,topic y where  y.is_business = 1 and w.topic=y.id and (y.id=? or y.code like ?))";
                    }
                }
                if(keyword!=null && keyword.length()>0) sql += " and a.title like ? ";
                if(creator!=null && creator.length()>0) sql += " and a.creator_name like ? ";
...
%>
        ...
        <input name="submitBtn" type="button" onClick="f_submit('down')" value="下架" class="button">            
        <% } %>
       <% if(status==8||status==9){ %>
        <input name="submitBtn" type="button" onClick="f_submit('up')" value="上架" class="button">            
        <% } %>        
...
%>
        <input name="toggleBtn" type="button" onClick="f_toggle()" value="产品目录" class="button">
        <input type="hidden" name="urlTo" value="productlist.jsp?page=<%=currpage%>" >
      </td>
    </tr>
    <tr height="25" style="padding-top:5px">
        ...
        <td>
            ...
            <select name="status" onchange="changestatus(this.value)">
                ...
                <option value="8" <% if(status==8) out.print("selected");%>>自动下架</option>
                <option value="9" <% if(status==9) out.print("selected");%>>手动下架</option>
            </select>
        </td>        
    </tr>
...
<%
        if (totalrows > 0)
        {
            ...
            if(table_name.equals("article"))
                sql = "select a.Id,a.title,a.creator_name creator,a.createdate,a.publishdate,b.id top_id,b.Name top_name,c.id site_id,c.name site_name from article a,topic b,site c Where a.topic=b.Id And a.siteid=c.Id  and b.is_business = 1   ";
            else
                sql = "select a.Id,a.title,a.createdate,a.publishdate,b.id top_id,b.Name top_name,c.id site_id,c.name site_name from " + table_name + " a,topic b,site c Where a.topic=b.Id And a.siteid=c.Id  and b.is_business = 1  ";     
            switch(status)
            {
                case 0: //查询草稿状态，默认加入作者判断
                    sql += " and a.state=?";
...
<script language="JavaScript" type="text/JavaScript">
  function openArt(siteid,topid,idvalue){
    ...
    document.frmOpen.action="productinfo.jsp";
