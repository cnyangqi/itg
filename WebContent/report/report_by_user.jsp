...
<%
    ...
    ResourceBundle bundle = ResourceBundle.getBundle("langs.jsp_report_by_user",user.GetLocale(), nps.core.Config.RES_CLASSLOADER);
...
<table width = "100% " border = "0" align = "center" cellpadding = "0" cellspacing = "1" class="titlebar">
  <tr height=30>
     <td width="20">&nbsp;</td>
     <td align=center width="120"><%=bundle.getString("REPORT_NAME")%></td>
...
<%
    ...
    try
    {
        ...
        sql = "Select a.Name,b.submit_rows,c.pass_rows,c.total_score From users a,";
        sql_submit = "Select creator,count(*) submit_rows From article";
        sql_pass = "Select creator,count(*) pass_rows,sum(score) total_score From article";
        ...
        if(begin_date!=null) whereclause_sub +=" and createdate>=to_date('"+begin_date+"','YYYY-MM-DD')";
        if(end_date!=null) whereclause_sub +=" and createdate<=to_date('"+end_date+"','YYYY-MM-DD')";

        sql =   sql
              + "(" + sql_submit + " where state>=1 " + whereclause_sub + " group by creator " +") b,"
              + "(" + sql_pass + " where state>1 " + whereclause_sub + " group by creator " +") c"
              + " where a.id = b.creator(+) And a.id=c.creator(+) And Exists(Select * From Site g, Dept h Where a.Dept = h.Id And h.Unit = g.Unit " + whereclause +")"
